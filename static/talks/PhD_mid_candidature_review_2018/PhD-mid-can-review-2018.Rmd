...
.pull-left[
### Batch scenario 
whole set of data is available, focus - complete events <br/> <br/> <br/>
...
- Propose an algorithm for early detection of nonstationarity (concept drift) <br/> <br/>
--
...
Feature Extraction $\Longrightarrow$ Instance Space
...
Principle Component Decomposition $\Longrightarrow$ 2D Feature Space
...
```{r  echo=FALSE, out.width = "60%", fig.cap="Figure: Extreme value distributions corresponding to m = 1; 10; 100; 1000, each describing where the maximum of m samples drawn from N(0; 1) will lie."}
...
```{r   echo=FALSE, out.width = "50%", fig.cap="Figure: Distribution of 1000 extremes generated from bivariate kernel density function with m=500"}
...
```{r   echo=FALSE, out.width = "50%",  fig.cap = "Figure: Distribution of transformed values"}
...
## Anomaly detection for high dimensional and streaming temperal data

---
#### **Project 3**: Anomaly detection for high dimensional and streaming temperal data
...
- Sort the resulted k nearest neighbour distances
...
- Search for for any significant large gap in the upper tail (Bottom up algorithm proposed by Schwarz (2008)) and thereby identify the  anomalous points
...
---
#### **Project 3**: Anomaly detection for high dimensional and streaming temperal data
### Limitations of HDoutliers algorithm  (Wilkinson, 2018)

- It normalizes data using the minimum  and the range which are not robust to ourliers 


- it uses only the nearest neighbour distances to discriminate outliers 


- it ignores the density of the data points after forming clusters using Leader algorithm and 


- it uses potential candidates for outliers for the calculation of the anomalous threshold
---

.pull-left[

### Advantages of the proposed algrithm 
...
- handle large data sets due to the use of a one-pass algorithm 


- does not require a training set to build the decision model


- deal with multimodal typical classes <br/><br/><br/><br/>

.tiny[LHS:HDoutliers  Vs RHS:stray]
]
.pull-right[
...
knitr::include_graphics("fig/P2_plot18.png")
```
]
...
.Large[<span style="color:blue">`stray`</span>  vs  <span style="color:blue">`oddstream`  </span>]
```{r   echo=FALSE, out.width = "100%"}
knitr::include_graphics("fig/P2_plot21.png")
```
